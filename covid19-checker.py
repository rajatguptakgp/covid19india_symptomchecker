...
os.makedirs('testing_sites',exist_ok=True)
...
def testing_sites_translator(df,df_lang,lang_id):
    ...
    columns_lang=list(df.columns)

    for i,label in enumerate(columns_lang):
        columns_lang[i]=translator.translate(label,dest=lang_id).text
...
def load_testing_facilities_data(person_state,lang):
    ...
    df_beds=pd.read_csv('data/HospitalBedsIndia.csv')
    hospital_beds=df_beds[df_beds[df_beds.columns[1]].values==person_state]
    hospital_beds.reset_index(drop=True,inplace=True)
    
    return df_state,df,hospital_beds
...
def ask_user_to_do_covid_test(lang):
    ...
    print()
    display(df_state_facilities)
    print()
    print('*'*60)
    print()
    
    print(translator.translate('Hospital Beds in your State',dest=lang_id).text)
    print()
    display(hospital_beds)
    print()
    print('*'*60)
    print()
    
    print(translator.translate('\nCOVID-19 Testing Sites in India',dest=lang_id).text)
    display(df_mass_test_facilities)
    print()
    print('*'*60)
    print()
...
if __name__ == '__main__':
    ...
    df_state_facilities, df_mass_test_facilities,hospital_beds = load_testing_facilities_data(person_state,lang)
    screen = Screen(lang)
    start_diagnoser()
